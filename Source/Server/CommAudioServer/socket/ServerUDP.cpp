...
boolean ServerUDP::InitSocket(int port) {
   ...
   qDebug() << "UDP InitSocket success on ip " << inet_ntoa(stLclAddr.sin_addr) << " port: " <<
               stLclAddr.sin_port;
   return true;
}
...
// Multicast UDP send, not used (see UDPSendThread)
...
boolean ServerUDP::InitData() {
    ...
    //if ((fstream = fopen("../Demo/Aero_Chord_ft_Anna_Yvette_-_Break_Them_(Monstep_Remix).wav", "wb"))== NULL){
